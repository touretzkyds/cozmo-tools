...
class ParentPilotEvent(StateNode):
    ...
    def start(self,event):
        ...
        self.parent.post_event(PilotEvent(event.status,*event.args))
...
class PilotBase(StateNode):
    ...
    class SendObject(StateNode):
        def start(self):
          ...
          if object.pose_confidence < 0:
              self.parent.post_event(PilotEvent(NotLocalized,object))
    ...
    class PilotExecutePlan(StateNode):
        ...
        def setup(self):
            ...
            # Code generated by genfsm on Tue Dec 31 06:15:41 2019:
...
class PilotToObject(PilotBase):
    ...
    def start(self, event=None):
        if isinstance(event,DataEvent):
            if isinstance(event.data,WorldObj):
                self.object = event.data
            else:
                raise ValueError('DataEvent to PilotToObject must be a WorldObj', event.data)
        super().start(event)
...
class PilotToPose(PilotBase):
    ...
    def setup(self):
        """
            planner: self.PilotRRTPlanner() =D=> exec
        ...
        datatrans7 .add_sources(planner) .add_destinations(exec)
...
class PilotFrustration(StateNode):
    ...
    class SayObject(Say):
        def start(self, event=None):
            try:
                self.text = self.parent.parent.object.name
            except:
                self.text = 'stuck'
            super().start(event)
    ...
    def setup(self):
        ...
        nulltrans2 .add_sources(launcher) .add_destinations(stopallmotors1)
        
        nulltrans3 = NullTrans() .set_name("nulltrans3")
        nulltrans3 .add_sources(stopallmotors1) .add_destinations(speak,turn)
