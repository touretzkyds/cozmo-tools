...
from .geometry import get_orientation_state
...
class GoToCube(StateNode):
    ...
    def pick_side(self, offset, use_world_map):
        ...
        if orientation == ORIENTATION_SIDEWAYS:
            if self.roll_cube:
                # if the orientation is sideways, only two valid roll cube sides
                if self.roll_target != ORIENTATION_SIDEWAYS:
                    self.side_index = 0 if (self.try_number == 0 or self.side_index > 2) else self.side_index
                    self.try_number = 2 if self.try_number == 0 else self.try_number
                    sides = (side1, side2)
                    sides = sorted(sides, key=lambda pt: (pt[0]-rx)**2 + (pt[1]-ry)**2)
            else:
    ...
    def setup(self):
        """
            ...
            exit_to_roll =F=> forward_to_cube_1a: self.ForwardToCube(15) =C=> success
            exit_to_roll =S=> success
        ...
        # Code generated by genfsm on Thu Oct 24 11:33:44 2019:
        ...
        failuretrans7 .add_sources(turn_to_cube1) .add_destinations(forward3)
        
        completiontrans11 = CompletionTrans() .set_name("completiontrans11")
        completiontrans11 .add_sources(forward3) .add_destinations(statenode2)
        ...
        completiontrans14 .add_sources(forward_to_cube_1a) .add_destinations(success)
        
        successtrans4 = SuccessTrans() .set_name("successtrans4")
        successtrans4 .add_sources(exit_to_roll) .add_destinations(success)
        ...
        completiontrans15 .add_sources(turn_to_cube2) .add_destinations(forward_to_cube2)
        
        completiontrans16 = CompletionTrans() .set_name("completiontrans16")
        completiontrans16 .add_sources(forward_to_cube2) .add_destinations(turn_to_cube3)
        
        completiontrans17 = CompletionTrans() .set_name("completiontrans17")
        completiontrans17 .add_sources(turn_to_cube3) .add_destinations(exit_to_roll2)
        ...
        successtrans5 .add_sources(exit_to_roll2) .add_destinations(success)
        
        completiontrans18 = CompletionTrans() .set_name("completiontrans18")
        completiontrans18 .add_sources(forward_to_cube3) .add_destinations(success)
        ...
        nulltrans19 .add_sources(failure) .add_destinations(choose_another_side)
        
        failuretrans11 = FailureTrans() .set_name("failuretrans11")
        failuretrans11 .add_sources(choose_another_side) .add_destinations(parentfails2)
...
class PickUpCube(StateNode):
    ...
    def setup(self):
        ...
        completiontrans19 .add_sources(fetch) .add_destinations(check_carry)
        ...
        completiontrans20 .add_sources(dropobject1) .add_destinations(goto_cube)
        
        failuretrans12 = FailureTrans() .set_name("failuretrans12")
        failuretrans12 .add_sources(check_carry) .add_destinations(goto_cube)
        ...
        failuretrans13 .add_sources(goto_cube) .add_destinations(fail)
        
        completiontrans21 = CompletionTrans() .set_name("completiontrans21")
        completiontrans21 .add_sources(goto_cube) .add_destinations(abortheadaction1)
        ...
        completiontrans22 .add_sources(raise_head) .add_destinations(raise_head2)
        
        completiontrans23 = CompletionTrans() .set_name("completiontrans23")
        completiontrans23 .add_sources(raise_lift,raise_head2) .add_destinations(verify)
        ...
        failuretrans14 .add_sources(verify) .add_destinations(statenode4)
        ...
        failuretrans15 .add_sources(verify2) .add_destinations(frustrated)
        ...
        failuretrans16 .add_sources(verify3) .add_destinations(frustrated)
        ...
        completiontrans24 .add_sources(satisfied) .add_destinations(final_raise,drop_head)
        
        completiontrans25 = CompletionTrans() .set_name("completiontrans25")
        completiontrans25 .add_sources(final_raise,drop_head) .add_destinations(parentcompletes2)
        ...
        completiontrans26 .add_sources(animationtriggernode1) .add_destinations(missed_cube)
        
        completiontrans27 = CompletionTrans() .set_name("completiontrans27")
        completiontrans27 .add_sources(missed_cube) .add_destinations(forward4)
        
        completiontrans28 = CompletionTrans() .set_name("completiontrans28")
        completiontrans28 .add_sources(forward4) .add_destinations(drop_lift,drop_head_low)
        
        completiontrans29 = CompletionTrans() .set_name("completiontrans29")
        completiontrans29 .add_sources(drop_lift) .add_destinations(backupmore)
        
        failuretrans17 = FailureTrans() .set_name("failuretrans17")
        failuretrans17 .add_sources(drop_lift) .add_destinations(backupmore)
        
        completiontrans30 = CompletionTrans() .set_name("completiontrans30")
        completiontrans30 .add_sources(backupmore,drop_head_low) .add_destinations(fail)
        
        completiontrans31 = CompletionTrans() .set_name("completiontrans31")
        completiontrans31 .add_sources(fail) .add_destinations(parentfails3)
...
class DropObject(StateNode):
    ...
    def setup(self):
        ...
        completiontrans32 .add_sources(setliftheight1) .add_destinations(check_carrying)
        
        failuretrans18 = FailureTrans() .set_name("failuretrans18")
        failuretrans18 .add_sources(check_carrying) .add_destinations(backup,lookdown)
        ...
        failuretrans19 .add_sources(lookdown) .add_destinations(head_angle_wait)
        
        completiontrans33 = CompletionTrans() .set_name("completiontrans33")
        completiontrans33 .add_sources(backup,lookdown) .add_destinations(head_angle_wait)
        ...
        completiontrans34 .add_sources(check_visible) .add_destinations(wrap_up)
        
        failuretrans20 = FailureTrans() .set_name("failuretrans20")
        failuretrans20 .add_sources(check_visible) .add_destinations(lookdown2)
        
        failuretrans21 = FailureTrans() .set_name("failuretrans21")
        failuretrans21 .add_sources(lookdown2) .add_destinations(wrap_up)
...
class RollCube(StateNode):
    ...
    def setup(self):
        """
            start: SetLiftHeight(1) =C=> Forward(40) =C=> put_arm
    
            put_arm: SetLiftHeight(0.65) =C=> {drop, back}
    
            drop: SetLiftHeight(0.25)
            back: Forward(-60)
    
            {drop, back} =C=> SetLiftHeight(0) =C=> Forward(-5) =C=> ParentCompletes()
            {drop, back} =F=> SetLiftHeight(0) =C=> Forward(-5) =C=> ParentFails()
        """
        
        # Code generated by genfsm on Thu Oct 24 11:33:44 2019:
        
        start = SetLiftHeight(1) .set_name("start") .set_parent(self)
        forward5 = Forward(40) .set_name("forward5") .set_parent(self)
        put_arm = SetLiftHeight(0.65) .set_name("put_arm") .set_parent(self)
        drop = SetLiftHeight(0.25) .set_name("drop") .set_parent(self)
        back = Forward(-60) .set_name("back") .set_parent(self)
        setliftheight2 = SetLiftHeight(0) .set_name("setliftheight2") .set_parent(self)
        forward6 = Forward(-5) .set_name("forward6") .set_parent(self)
        ...
        forward7 = Forward(-5) .set_name("forward7") .set_parent(self)
        ...
        completiontrans35 .add_sources(start) .add_destinations(forward5)
        
        completiontrans36 = CompletionTrans() .set_name("completiontrans36")
        completiontrans36 .add_sources(forward5) .add_destinations(put_arm)
        
        completiontrans37 = CompletionTrans() .set_name("completiontrans37")
        completiontrans37 .add_sources(put_arm) .add_destinations(drop,back)
        
        completiontrans38 = CompletionTrans() .set_name("completiontrans38")
        completiontrans38 .add_sources(drop,back) .add_destinations(setliftheight2)
        
        completiontrans39 = CompletionTrans() .set_name("completiontrans39")
        completiontrans39 .add_sources(setliftheight2) .add_destinations(forward6)
        
        completiontrans40 = CompletionTrans() .set_name("completiontrans40")
        completiontrans40 .add_sources(forward6) .add_destinations(parentcompletes4)
        
        failuretrans22 = FailureTrans() .set_name("failuretrans22")
        failuretrans22 .add_sources(drop,back) .add_destinations(setliftheight3)
        
        completiontrans41 = CompletionTrans() .set_name("completiontrans41")
        completiontrans41 .add_sources(setliftheight3) .add_destinations(forward7)
        
        completiontrans42 = CompletionTrans() .set_name("completiontrans42")
        completiontrans42 .add_sources(forward7) .add_destinations(parentfails4)
        
        return self


class RollingCube(StateNode):
    def __init__(self, object=None, orientation=None):
        super().__init__()
        self.object = object
    ...
    class CheckOrientation(StateNode):
        def start(self,event=None):
            ...
            super().start(event)
            if self.parent.target_orientation == self.parent.orientation:
                self.post_completion()
            else:
                print('current orientation:', self.parent.orientation)
                self.post_failure()
    ...
    class CheckCounts(StateNode):
        def start(self,event=None):
            ...
            if not self.parent.target_orientation:
                ...
                # print('robot_angle: %.2f' % robot_angle)

                z = min([math.pi/2, 0, -math.pi/2], key=lambda val:abs(val-z))

                cube_rotate_angle = 0
                new_robot_angle = 0
                if z == -math.pi/2:
                    pattern = 1
                    cube_rotate_angle = wrap_angle(x - math.pi/2)
                    ...
                    if min([0, -math.pi, math.pi], key=lambda val:abs(val-x)) == 0:
                        pattern = 2
                        cube_rotate_angle = wrap_angle(y + math.pi)
                    else:
                        pattern = 4
                        cube_rotate_angle = y
                    new_robot_angle = wrap_angle(robot_angle + cube_rotate_angle)
                ...
                # print('cube_rotate_angle: %.2f' % cube_rotate_angle)

                possible_angles = [-math.pi, -math.pi/2, 0, math.pi/2, math.pi]
                facing_side = None
                new_robot_angle = min(possible_angles, key=lambda val:abs(val-new_robot_angle))
                if new_robot_angle == 0:
                    facing_side = SIDEWAYS_FRONT
                elif abs(new_robot_angle) == math.pi:
    ...
    def setup(self):
        """
            ...
            check_cube_pose: self.CheckCubePoseValidOnce()
            ...
            goto_cube: GoToCube()
            goto_cube =PILOT=> goto_cube_pilot: ParentPilotEvent() =N=> Print('goto_cube has pilot.') =N=> decide_roll_counts
            ...
            decide_roll_counts =D(0)=> Print('No way to achieve from this side') =C=> check_cube_pose
            decide_roll_counts =D(1)=> Print('Roll once') =C=> Forward(-10) =C=> roll1
            decide_roll_counts =D(2)=> Print('Roll twice') =C=> Forward(-10) =C=> roll2
            decide_roll_counts =D(3)=> Print('Roll thrice') =C=> Forward(-10) =C=> roll3
    
            roll1: RollCube()
            roll1 =C=> Forward(-30) =C=> Print('Checking the new orientation...') =T(1)=> check_cube_pose
            roll1 =F=> try_again
        ...
        looker = LookAtObject() .set_name("looker") .set_parent(self)
        check_cube_pose = self.CheckCubePoseValidOnce() .set_name("check_cube_pose") .set_parent(self)
        ...
        goto_cube = GoToCube() .set_name("goto_cube") .set_parent(self)
        goto_cube_pilot = ParentPilotEvent() .set_name("goto_cube_pilot") .set_parent(self)
        print14 = Print('goto_cube has pilot.') .set_name("print14") .set_parent(self)
        print15 = Print('goto_cube has failed.') .set_name("print15") .set_parent(self)
        print16 = Print('goto_cube has succeeded.') .set_name("print16") .set_parent(self)
        decide_roll_counts = self.CheckCounts() .set_name("decide_roll_counts") .set_parent(self)
        print17 = Print('No way to achieve from this side') .set_name("print17") .set_parent(self)
        print18 = Print('Roll once') .set_name("print18") .set_parent(self)
        forward8 = Forward(-10) .set_name("forward8") .set_parent(self)
        print19 = Print('Roll twice') .set_name("print19") .set_parent(self)
        forward9 = Forward(-10) .set_name("forward9") .set_parent(self)
        print20 = Print('Roll thrice') .set_name("print20") .set_parent(self)
        forward10 = Forward(-10) .set_name("forward10") .set_parent(self)
        roll1 = RollCube() .set_name("roll1") .set_parent(self)
        forward11 = Forward(-30) .set_name("forward11") .set_parent(self)
        ...
        completiontrans43 .add_sources(start) .add_destinations(looker,check_cube_pose)
        
        completiontrans44 = CompletionTrans() .set_name("completiontrans44")
        completiontrans44 .add_sources(check_cube_pose) .add_destinations(check_orientation)
        
        failuretrans23 = FailureTrans() .set_name("failuretrans23")
        failuretrans23 .add_sources(check_cube_pose) .add_destinations(parentfails5)
        
        completiontrans45 = CompletionTrans() .set_name("completiontrans45")
        completiontrans45 .add_sources(check_orientation) .add_destinations(parentcompletes5)
        
        failuretrans24 = FailureTrans() .set_name("failuretrans24")
        failuretrans24 .add_sources(check_orientation) .add_destinations(goto_cube)
        
        pilottrans6 = PilotTrans() .set_name("pilottrans6")
        pilottrans6 .add_sources(goto_cube) .add_destinations(goto_cube_pilot)
        
        nulltrans27 = NullTrans() .set_name("nulltrans27")
        nulltrans27 .add_sources(goto_cube_pilot) .add_destinations(print14)
        
        nulltrans28 = NullTrans() .set_name("nulltrans28")
        nulltrans28 .add_sources(print14) .add_destinations(decide_roll_counts)
        
        failuretrans25 = FailureTrans() .set_name("failuretrans25")
        failuretrans25 .add_sources(goto_cube) .add_destinations(print15)
        
        nulltrans29 = NullTrans() .set_name("nulltrans29")
        nulltrans29 .add_sources(print15) .add_destinations(try_again)
        
        completiontrans46 = CompletionTrans() .set_name("completiontrans46")
        completiontrans46 .add_sources(goto_cube) .add_destinations(print16)
        
        nulltrans30 = NullTrans() .set_name("nulltrans30")
        nulltrans30 .add_sources(print16) .add_destinations(decide_roll_counts)
        
        datatrans2 = DataTrans(0) .set_name("datatrans2")
        datatrans2 .add_sources(decide_roll_counts) .add_destinations(print17)
        
        completiontrans47 = CompletionTrans() .set_name("completiontrans47")
        completiontrans47 .add_sources(print17) .add_destinations(check_cube_pose)
        
        datatrans3 = DataTrans(1) .set_name("datatrans3")
        datatrans3 .add_sources(decide_roll_counts) .add_destinations(print18)
        
        completiontrans48 = CompletionTrans() .set_name("completiontrans48")
        completiontrans48 .add_sources(print18) .add_destinations(forward8)
        
        completiontrans49 = CompletionTrans() .set_name("completiontrans49")
        completiontrans49 .add_sources(forward8) .add_destinations(roll1)
        
        datatrans4 = DataTrans(2) .set_name("datatrans4")
        datatrans4 .add_sources(decide_roll_counts) .add_destinations(print19)
        
        completiontrans50 = CompletionTrans() .set_name("completiontrans50")
        completiontrans50 .add_sources(print19) .add_destinations(forward9)
        
        completiontrans51 = CompletionTrans() .set_name("completiontrans51")
        completiontrans51 .add_sources(forward9) .add_destinations(roll2)
        
        datatrans5 = DataTrans(3) .set_name("datatrans5")
        datatrans5 .add_sources(decide_roll_counts) .add_destinations(print20)
        
        completiontrans52 = CompletionTrans() .set_name("completiontrans52")
        completiontrans52 .add_sources(print20) .add_destinations(forward10)
        
        completiontrans53 = CompletionTrans() .set_name("completiontrans53")
        completiontrans53 .add_sources(forward10) .add_destinations(roll3)
        
        completiontrans54 = CompletionTrans() .set_name("completiontrans54")
        completiontrans54 .add_sources(roll1) .add_destinations(forward11)
        
        completiontrans55 = CompletionTrans() .set_name("completiontrans55")
        completiontrans55 .add_sources(forward11) .add_destinations(print21)
        
        timertrans13 = TimerTrans(1) .set_name("timertrans13")
        timertrans13 .add_sources(print21) .add_destinations(check_cube_pose)
        
        failuretrans26 = FailureTrans() .set_name("failuretrans26")
        failuretrans26 .add_sources(roll1) .add_destinations(try_again)
        
        completiontrans56 = CompletionTrans() .set_name("completiontrans56")
        completiontrans56 .add_sources(roll2) .add_destinations(roll1)
        
        failuretrans27 = FailureTrans() .set_name("failuretrans27")
        failuretrans27 .add_sources(roll2) .add_destinations(try_again)
        
        completiontrans57 = CompletionTrans() .set_name("completiontrans57")
        completiontrans57 .add_sources(roll3) .add_destinations(roll2)
        
        failuretrans28 = FailureTrans() .set_name("failuretrans28")
        failuretrans28 .add_sources(roll3) .add_destinations(try_again)
        
        successtrans12 = SuccessTrans() .set_name("successtrans12")
        successtrans12 .add_sources(try_again) .add_destinations(check_cube_pose)
        
        failuretrans29 = FailureTrans() .set_name("failuretrans29")
        failuretrans29 .add_sources(try_again) .add_destinations(fail)
        
        completiontrans58 = CompletionTrans() .set_name("completiontrans58")
        completiontrans58 .add_sources(fail) .add_destinations(parentfails6)
