...
class DoorPass(StateNode):
    ...
    def setup(self):
        """
            ...
            turn_to_gate1: self.TurnToGate(150) =C=> StateNode() =T(0.5)=> forward_to_gate1
            forward_to_gate1: self.ForwardToGate(150) =C=> StateNode() =T(0.5)=> {look_up, turn_to_gate2}
    
            # If we're carrying a cube, should we lower the lift so we can see?
            look_up: SetHeadAngle(35)
    
            turn_to_gate2: self.TurnToGate(70) =C=> StateNode() =T(0.5)=> turn_to_marker1
    
            turn_to_marker1: self.TurnToMarker() =C=>
                self.ForwardToGate(70) =C=> SetHeadAngle(40) =C=> StateNode() =T(0.5)=> turn_to_marker2
    
            turn_to_marker2: self.TurnToMarker() =C=> StateNode() =T(0.5)=>  {lower_head, through_door}
        ...
        # Code generated by genfsm on Tue Jan  8 22:18:46 2019:
