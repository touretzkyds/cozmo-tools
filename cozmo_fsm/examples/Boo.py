...
class HeadAndLiftGesture(StateNode):
    """Move lift and head simultaneously. Finish when head movement completes."""
    def setup(self):
        """
            launch: StateNode() =N=> {move_lift, move_head}
            move_lift: MoveLift(-3)
            move_head: SetHeadAngle(cozmo.robot.MAX_HEAD_ANGLE)
    
            {move_lift, move_head} =C(1)=> ParentCompletes()
        """
        
        # Code generated by genfsm on Fri Dec 23 11:49:18 2016:
        ...
        nulltrans1 .add_sources(launch) .add_destinations(move_lift,move_head)
        
        completiontrans1 = CompletionTrans(1) .set_name("completiontrans1")
        completiontrans1 .add_sources(move_lift,move_head) .add_destinations(parentcompletes1)
